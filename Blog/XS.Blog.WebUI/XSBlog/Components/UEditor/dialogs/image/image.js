...
(function () {
    ...
    function initButtons() {
        ...
        $("#chkAddToAlbum").click(function() {
            $("#albumlist").toggle();
        });

        var url = editor.getOpt('serverUrl') + '?action=getAlbumList',
            ...
            'onsuccess': function(r) {
                try {
                    ...
                    if (json.state == 'SUCCESS') {
                        var list = json.list;
                        var sltAlbumList = $("#albumlist");

                        for (var i = 0; i < list.length; i++) {
                                sltAlbumList.append("<option value='" + list[i].id + "'>" + list[i].name + "</option>");
                        }
                        if (list.length == 0) {
                            sltAlbumList.append("<option value='0'>没有相册</option>");
                        }
                    }
                } catch (e) {
                }
    ...
    function getImageSize() {
        var imageSize = $G("imageSize");
        var value = imageSize.options[imageSize.selectedIndex].value;
        return value;
    }
    /* 获取选中的相册Id */
    function getAlbumId() {
        var albumlist = $G("albumlist");
        if ($("#albumlist").is(':visible'))
            return albumlist.options[albumlist.selectedIndex].value;
        return "";
    }
