...
namespace XS.Framework.Common.Extension
{
    ...
    public static class StringExtension
    {
        ...
        /// 需要转义的字符正则
        /// </summary>
        private static readonly Regex TransferredRule = new Regex(@"(""|\\)", RegexOptions.Compiled);

        /// <summary>
        ///  空白字符正则
        /// </summary>
        private static readonly Regex SpaceRule = new Regex(@"\s", RegexOptions.Compiled);

        /// <summary>
        ///  HTML标签正则
        /// </summary>
        private static readonly Regex HtmlTagRule = new Regex(@"<[^>]*>", RegexOptions.Compiled);
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static string IfNotNullAndEmpty(this string str, Func<string> value)
        {
            if (!string.IsNullOrEmpty(str))
                return value();
        ...
        /// <param name="noValue"></param>
        /// <returns></returns>
        public static string IfNotNullAndEmpty(this string str, Func<string, string> yesValue,
            Func<string, string> noValue)
        {
            if (!string.IsNullOrEmpty(str))
                return yesValue(str);
        ...
        public static string Remove(string str, params string[] deleteStrs)
        {
            ...
            var sb = new StringBuilder(str);
            foreach (string i in deleteStrs)
            {
        ...
        public static string RemoveSpaceString(this string str)
        {
            ...
            return SpaceRule.Replace(str, string.Empty);
        }
        ...
        private static int[] intSplit(string str, string separator, int? defaultValue)
        {
            if (str == null)
                throw new ArgumentNullException("str");
            ...
            string[] numbersStringArray = str.Split(new[] {separator},
                StringSplitOptions.RemoveEmptyEntries);
            var numbers = new List<int>();

            foreach (string item in numbersStringArray)
            {
                int i;
        ...
        public static string ToJavascriptString(this string str, bool addDoubleQuotes)
        {
            ...
            str = TransferredRule.Replace(str, "\\$1");
            var sb = new StringBuilder(str);
            sb.Replace("\r", "\\r");
            sb.Replace("\n", "\\n");
        ...
        public static string RemoveHtml(this string str)
        {
            ...
            return HtmlTagRule.Replace(str, string.Empty);
        }
        ...
        public static bool IsURL(this string strUrl)
        {
            return Regex.IsMatch(strUrl,
                @"^(http|https|ftp)\://([a-zA-Z0-9\.\-]+(\:[a-zA-Z0-9\.&%\$\-]+)*@)*((25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9])\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0)\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[1-9]|0)\.(25[0-5]|2[0-4][0-9]|[0-1]{1}[0-9]{2}|[1-9]{1}[0-9]{1}|[0-9])|localhost|([a-zA-Z0-9\-]+\.)*[a-zA-Z0-9\-]+\.(com|edu|gov|int|mil|net|org|biz|arpa|info|name|pro|aero|coop|museum|[a-zA-Z]{1,10}))(\:[0-9]+)*(/($|[a-zA-Z0-9\.\,\?\'\\\+&%\$#\=~_\-]+))*$");
        }
        ...
        {
            const string pattrn = @"^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$";
        ...
        public static char? GetFirstLetter(this string input)
        {
            ...
            if (arrCN.Length > 1)
            {
                ...
                {
                    return Encoding.Default.GetChars(new[] {(byte) (area)})[0];
                }
                int pos = arrCN[1];
                int code = (area << 8) + pos;
                int[] areacode =
                ...
                for (int i = 0; i < 26; i++)
                {
                    ...
                    {
                        return Encoding.Default.GetChars(new[] {(byte) (65 + i)})[0];
                    }
                }
            }
            return null;
